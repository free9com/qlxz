...
本指南将详细介绍如何在本地环境中设置Supabase开发环境，包含项目所需的数据库表结构。

## 系统要求

- Docker (version 20.10.0或更高)
- Docker Compose (version 2.0.0或更高)
- 至少4GB可用内存
...
services:
  supabase-db:
    ...
    ports:
      - "5432:5432"
    volumes:
      - ./init_supabase.sql:/docker-entrypoint-initdb.d/init.sql
      - supabase_db_data:/var/lib/postgresql/data
    restart: unless-stopped

  supabase-auth:
    image: supabase/gotrue:v2.75
  ...
  supabase-rest:
    ...
    environment:
      ...
      PGRST_OPENAPI_SERVER_PROXY_URI: http://supabase-db:5432/postgres
  ...
  supabase-storage:
    ...
    depends_on:
      - supabase-db
    environment:
      POSTGRES_URL: postgres://postgres:postgres@supabase-db:5432/postgres
...
- Auth服务: `localhost:9999`
- REST API: `localhost:3000`
- Storage API: `localhost:5000`

## 连接到数据库

使用以下凭据连接到PostgreSQL数据库：
...
1. 如果遇到权限错误，请确保当前用户在docker组中：
   ```bash
   sudo usermod -aG docker $USER
   # 注销并重新登录以使更改生效
   ```

2. 如果端口已被占用，请修改docker-compose.yml中的端口映射。

3. 如果数据库初始化失败，请检查init_supabase.sql文件的语法。

## 项目结构说明

创建的数据库包含以下主要表：

- **项目相关**: projects, project_members, documents
- **任务相关**: tasks, bbs_posts
- **LLM处理相关**: llm_messages, task_periods, task_quantity, task_materials, task_tech_disclosure, task_safe_disclosure
- **审批流程相关**: approval_flows, approval_flows_inst, approval_nodes, approval_node_inst, form_elements, form_element_val
- **用户相关**: users, positions, departments, roles
- **公司表**: companys